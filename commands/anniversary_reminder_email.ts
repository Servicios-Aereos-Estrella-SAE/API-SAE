...
export default class AnniversaryReminderEmail extends BaseCommand {
  static commandName = 'anniversary:reminder-email'
  static description = 'Send anniversary reminder emails to HR users about employees celebrating their work anniversary today'
  ...
  async run() {
    this.logger.info('🎉 Starting anniversary reminder email process...')
    ...
    try {
      ...
      // Buscar empleados que cumplen aniversario hoy y están activos, filtrados por business access
      ...
      this.logger.info(`🎂 Found ${anniversaryEmployees.length} employees celebrating their work anniversary today (filtered by business access)`)

      if (anniversaryEmployees.length === 0) {
        this.logger.info('📭 No anniversary employees found for today in the current business units')
        return
      }
      ...
      for (const hrUser of hrUsers) {
        try {
          await this.sendAnniversaryReminderEmail(hrUser, anniversaryEmployees, systemSetting)
          emailsSent++
          this.logger.info(`✅ Anniversary reminder email sent to: ${hrUser.person.personFirstname} ${hrUser.person.personLastname} (${hrUser.person.personEmail})`)
        } catch (error) {
          emailsFailed++
      ...
      this.logger.info('📊 Anniversary reminder email process completed:')
      ...
      this.logger.info(`   🎂 Anniversary employees: ${anniversaryEmployees.length}`)

    } catch (error) {
      this.logger.error(`❌ Error in anniversary reminder email process: ${error.message}`)
    }
  }

  private async sendAnniversaryReminderEmail(hrUser: User, anniversaryEmployees: Employee[], systemSetting: SystemSetting) {
    const person = hrUser.person

    // Crear el HTML del email
    const emailHtml = this.generateAnniversaryReminderEmailHtml(
    ...
    await mail.send((message) => {
      message
        .to(hrUser.userEmail)
        .subject(`🎂 Recordatorio: Empleados que cumplen aniversario hoy - ${systemSetting.systemSettingTradeName!}`)
        .html(emailHtml)
    })
  }

  private generateAnniversaryReminderEmailHtml(
    hrFirstName: string,
    hrLastName: string,
